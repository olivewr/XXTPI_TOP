...
SELECT segment1 "Entity Unique Identifier",
       segment3 "Account Number",
       NULL "Key3",
       NULL "Key4",
       NULL "Key5",
       NULL "Key6",
       ...
       sum(fuctional_outstanding_balance) "Subledger Account Balance"
       from (
select cust.customer_name customer_name,
    rat.trx_number,
    rat.TRX_DATE,
    gcc.segment1,
    to_char(last_day(add_months(sysdate,-1)),'MM/DD/YYYY') "Period End Date",
    ...
    from  ra_customer_trx_all rat,
      ...
      and    gld.org_id in (103, 162, 164, 166, 168, 170, 183, 184, 282, 342, 382, 383, 422, 481)
...
SELECT segment1 "Entity Unique Identifier",
       ...
       NULL "SL Reporting Balance",
       NULL "SL Alternate Balance",
       sum(amount_dr)-sum(amount_cr) "Subledger Account Balance"
       from (
