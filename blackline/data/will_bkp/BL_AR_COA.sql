...
--execute MO_GLOBAL.SET_POLICY_CONTEXT('S',582);
...
SELECT segment1 seg1,
       ...
       NULL "Key9",
       NULL "Key10",
       to_char(last_day(add_months(sysdate,-2)),'MM/DD/YYYY') ped,
       NULL "SL Reporting Balance",
       NULL "SL Alternate Balance",
       sum(nvl(fuctional_outstanding_balance, 0)) sab
       from (
...
AND hl_bill.location_id       =hps_bill.location_id
     ...
     -- and     rat.BILL_TO_CUSTOMER_ID = cust.CUSTOMER_ID
      and    gld.gl_date<= last_day(add_months(sysdate,-2))
      and    gld.account_class in ('REC')
      and    gld.latest_rec_flag  = 'Y'
      -- Taraj update on 6/11/18 Switz, Denm and IA-2 is added
      and    gld.org_id in (103, 162, 164, 166, 168, 170, 183, 184, 282, 342, 382, 383, 422,442, 481, 542, 562, 582, 585, 622, 624, 625, 728, 748, 768,848,808)
     and rat.complete_flag='Y'
     and rat.TRX_NUMBER <> '15646'
     and gcc.segment1 not in ('100')
     and gcc.segment3 not in ('201100','201900')
)--where Orj_outstanding_balance<>0 ( commented because it is not replacing with 0)
group by SEGMENT1, SEGMENT3, SEGMENT5
--group by seg1,seg3, seg5
UNION
SELECT segment1 seg1,
       segment3 seg3,
       segment5 seg5,
  ...
  select
   ...
   -- f.TRX_NUMBER,
    ...
    f.TRX_DATE,
    gl.segment1,
    to_char(last_day(add_months(sysdate,-2)),'MM/DD/YYYY') Period_End_Date,
    gl.segment3,
    gl.segment5,
    rap.GL_DATE,
    gl.CONCATENATED_SEGMENTS,
    NVL (amount_dr, 0)* nvl(cr.exchange_rate,2) "AMOUNT_DR",
          NVL (amount_cr, 0)* nvl(cr.exchange_rate,1) "AMOUNT_CR",
          NVL (acctd_amount_dr, 0)* nvl(cr.exchange_rate,1) "ACCTD_AMOUNT_DR",
          NVL (acctd_amount_cr, 0)* nvl(cr.exchange_rate,1) "ACCTD_AMOUNT_CR",
  ...
  FROM ar_cash_receipts_v cr,
    ...
    WHERE     cr.CASH_RECEIPT_ID = rap.CASH_RECEIPT_ID
          ...
          AND rap.GL_DATE <= last_day(add_months(sysdate,-2))
         -- and cr.CASH_RECEIPT_ID =66095
          AND GL.SEGMENT3 = 111970
          --AND GL.SEGMENT1 = 11
          and gl.segment1 not in ('100')
          )
group by SEGMENT1, SEGMENT3, segment5
union
SELECT segment1 seg1,
