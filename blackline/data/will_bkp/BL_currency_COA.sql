...
SELECT   distinct       
   ...
   WHERE 1=1
     ...
     AND to_char(CONVERSION_DATE,'MON-YY')  =  TO_CHAR ((LAST_DAY (ADD_MONTHS (sysdate,-2))), 'MON-YY')
     AND r.to_currency in ('CNY','TRY','MXN', 'DKK', 'CHF')
     AND to_char(end_date,'MM/DD/YYYY')  =  to_char(CONVERSION_DATE,'MM/DD/YYYY')
     AND to_char(end_date,'MM/DD/YYYY')  =  TO_CHAR ((LAST_DAY (ADD_MONTHS (sysdate,-2))), 'MM/DD/YYYY')
     AND (r.conversion_type like  'Period End%' or r.conversion_type = '1140')
UNION ALL
SELECT 'USD', 1, TO_CHAR ((LAST_DAY (ADD_MONTHS (sysdate,-2))), 'MM/DD/YYYY') period_end_date, 'D'
from dual;
